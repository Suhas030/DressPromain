                    ...
                    try {
                        ...
                        return (
                            <div key={index} 
                                ...
                                title={color[0]}
                            >
                                <span className="fc-color-percentage" style={{
                                    fontSize: '12px',
                                    fontWeight: 'bold'
                                }}>
                                    {percentage}%
                        ...
                        {!recommendations.length ? (
                                            ...
                                            <button 
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    if (previewImages.length > 0) {
                                                        try {
                                                            const img = previewImages[0];
                                                            console.log("Running comprehensive model analysis on first image...");
                                                            const analysis = await ModelService.analyzeModelOutput(img.element);
                                                            console.log("Analysis complete:", analysis);
                                                            
                                                            if (analysis && analysis.processedDetections) {
                                                                alert(`Found ${analysis.processedDetections.length} detections: ${
                                                                    analysis.processedDetections.map(d => `${d.class} (${(d.confidence*100).toFixed(1)}%)`).join(', ')
                                                                }`);
                                                            } else {
                            ...
                            <div className="fc-results">
                                <div className="fc-style-summary">
                                    ...
                                    {styleAnalysis && styleAnalysis.dominant_colors && (
                                        <>
